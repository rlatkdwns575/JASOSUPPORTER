...
import 'package:chatgptmini/master_resume_tip_sheet.dart';
import 'package:flutter/material.dart';

/// Q1~Q6 문항 표시 + 문항별 초안 입력, 전체 초고 첨삭 탭.
class MasterResumeWorkspace extends StatefulWidget {
  ...
  @override
  State<MasterResumeWorkspace> createState() => _MasterResumeWorkspaceState();
}

class _MasterResumeWorkspaceState extends State<MasterResumeWorkspace> {
  MasterResumeTipContent? _openTip;
  final Map<int, Set<String>> _selectedExperienceIdsByQuestion = {};
  ...
  List<String> _selectedIdsForQuestion(int index) {
    return (_selectedExperienceIdsByQuestion[index] ?? <String>{}).toList(growable: false);
  }
  ...
  void _toggleExperienceForQuestion(int questionIndex, String experienceId) {
    setState(() {
      final Set<String> selected = _selectedExperienceIdsByQuestion.putIfAbsent(questionIndex, () => <String>{});
      if (!selected.add(experienceId)) {
        selected.remove(experienceId);
      }
    });
  }
  ...
  Future<void> _loadVersion(int tabIndex) async {
    ...
    setState(() {
      _selectedExperienceIdsByQuestion[tabIndex] = sourceIds.toSet();
    });
  }
    ...
    if (widget.availableExperiences.isEmpty) {
      return const AppCard(
        ...
        child: SectionHeader(
          ...
          subtitle: "경험·스펙 모드에서 ‘경험 카드로 저장’을 먼저 누르면 자소서 문항에 바로 연결할 수 있습니다.",
        ...
        child: Column(
          ...
          children: [
            SectionHeader(
              ...
              subtitle: "문항별로 선택하면 AI 초안에 우선 반영됩니다.",
              icon: Icons.inventory_2_outlined,
            ),
  ...
  Widget _buildQuestionExperienceSelector(int questionIndex) {
    ...
    final Set<String> selected = _selectedExperienceIdsByQuestion[questionIndex] ?? <String>{};
          ...
          children: [
            SectionHeader(
              ...
              subtitle: selected.isEmpty
                  ? "선택하지 않으면 전체 저장 경험이 참고 데이터로만 전달됩니다."
                  : "선택한 경험 ${selected.length}개가 초안 요청에 우선 반영됩니다.",
        ...
        Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "문항 초안·첨삭 시 직무 맞춤으로 참고합니다. 예: 해운 물류 영업, 백엔드 개발.",
                style: TextStyle(fontSize: 12, height: 1.35, color: AppColors.onSurfaceVariant),
              ),
              const SizedBox(height: 8),
              TextField(
                  ...
                  ? () async {
                      ...
                      await ExportService.pickFormatAndSaveRequest(
                        ...
                        request: ExportRequest(
                          ...
                          artifactType: ExportArtifactType.masterEssay,
                          title: "Q1~Q6 합본",
                          sourceIds: _selectedExperienceIdsByQuestion.values
      ...
      child: Column(
        ...
        children: [
          AppCard(
            ...
            child: SectionHeader(
              title: q.title,
              subtitle: "${q.body} (${q.charHint}, 공백 포함)",
      ...
      child: Column(
        ...
        children: [
          AppCard(
            ...
            child: SectionHeader(
              title: "전체 초고 첨삭",
              subtitle: "작성해 둔 전체 초고를 붙여 넣거나, 아래 채팅·‘자료·복붙’에 둔 내용을 참고해 첨삭을 요청하세요.",
          ...
          AppCard(
            ...
            child: TextField(
              ...
              style: const TextStyle(color: AppColors.onSurface),
              decoration: InputDecoration(
                hintText: "전체 초고를 여기에 붙이거나, 없으면 없음. (짧게만 쓸 경우 오른쪽 채팅에만 적어도 됩니다.)",
                hintStyle: const TextStyle(color: AppColors.outline),
                filled: true,
                fillColor: AppColors.surfaceContainerLowest,
                enabledBorder: OutlineInputBorder(
