...
export 'package:chatgptmini/features/experience/experience_field_style.dart'
    show globalFieldDecoration;

/// 글로벌 경험 소분류 입력 화면 공통 셸.
class GlobalInputShell extends StatefulWidget {
  const GlobalInputShell({
    super.key,
  ...
  @override
  State<GlobalInputShell> createState() => _GlobalInputShellState();
}

class _GlobalInputShellState extends State<GlobalInputShell> {
  bool _saving = false;
  ...
  Widget build(BuildContext context) {
    ...
    return ExperienceInputChrome(
      title: subtype.title,
      categoryLabel: '글로벌 경험',
      categoryIcon: Icons.public_outlined,
      backLabel: '세부 유형으로 돌아가기',
      onBack: () => context.go(AppRoutes.experienceFormCategory('global')),
      form: widget.form,
      onSubmit: _save,
      submitLabel:
